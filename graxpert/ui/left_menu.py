...
class ExtractionMenu(CollapsibleMenuFrame):
    def __init__(self, parent, **kwargs):
        ...
        self.auto_pts = tk.BooleanVar()
        self.auto_pts.set(graxpert.prefs.bg_auto_pts_option)
        self.auto_pts.trace_add("write", lambda a, b, c: eventbus.emit(AppEvents.BG_AUTO_SELECTION_CHANGED, {"bg_auto_selection_option": self.auto_pts.get()}))
    ...
    def create_children(self):
        ...
        self.auto_select_pts_switch = GraXpertCheckbox(self.sub_frame, width=default_label_width, text=_("Intelligent generation"), variable=self.auto_pts)
        tooltip.Tooltip(self.auto_select_pts_switch, text=tooltip.bg_auto_text)
    ...
    def place_children(self, event=None):
        ...
        self.auto_select_pts_switch.grid_forget()
        ...
        if not self.interpol_type.get() == "AI":
            ...
            self.auto_select_pts_switch.grid(column=1, row=next_row(), pady=pady, sticky=tk.EW)
